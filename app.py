...
with st.sidebar:
    ...
    if 'df' in st.session_state:
        st.success(f"✅ Data loaded ({len(st.session_state['df'])} responden)")
    else:
...
def load_default_data():
    """Load data default dari file Excel di repository"""
    default_file = "Data_Kuesioner_Kosan_Spk (1).xlsx"
...
if menu == "🏠 Dashboard":
    ...
    # Coba load data default jika belum ada
    if 'df' not in st.session_state:
        ...
        if default_df is not None:
            ...
            st.info("📂 Belum ada data. Silahkan upload file Excel.")
    ...
    if uploaded_file is not None:
        ...
        if 'df' in st.session_state:
            ...
            st.success(f"✅ Data sudah diupload! ({len(df)} responden)")
